...
Dashboard package release: 1.0.10
...
notebooks\vrp_sofr_eod_update_v1.py
...
3. Refresh and validate the canonical FRED SOFR cache before implied variance.
4. Record first_changed_sofr_date from the SOFR update manifest when FRED adds or revises observations.
5. Scan all components for recent and interior missing dates.
6. Resolve the earliest recalculation date from the detected component gap, first_changed_sofr_date, and any explicit incident-recovery override.
7. Refresh the VIX-style implied-variance surface and option-chain cache dependencies. When SOFR changed, replace every affected date x tenor row from the earliest changed SOFR date forward.
8. Rebuild canonical SPY EOD, realized volatility, and RV21D through the target date.
9. Extend the accepted long-warmup Wilder RSI14 history from its stored recursive state.
10. Update Corsi intraday realized-variance sources.
11. Rebuild the locked feature panel from before the earliest affected date when necessary.
12. Reconstruct the annual expanding intraday Ridge forecast using:
...
13. Compare the full historical overlap with the locked intraday forecast reference at 1e-8 tolerance.
14. Recalculate log VRP and prior-only 63/252-session z-scores from complete history.
15. Apply exact Hybrid v2 Core and Secondary thresholds, sizes, and the size-led daily selector.
16. Stage all dashboard products.
17. Publish atomically only after the final hard health gate passes.
18. Restore every pre-run canonical file and remove newly created canonical outputs if the final gate fails.

SOFR production contract
------------------------
Canonical cache: data\external\fred_sofr_history.csv
Source: official FRED SOFR CSV.
The SOFR updater validates date uniqueness, finite rates, sanity bounds, non-truncation, and post-write readback.
The health gate requires the latest observation expected to have been published by the prior completed XNYS
session. It does not use an arbitrary calendar-age threshold. A new or revised historical SOFR observation
causes implied variance and downstream signals to be rebuilt from first_changed_sofr_date.
...
--recalc-start YYYYMMDD      Controlled incident-recovery override for the earliest downstream recalculation date.
                             Omit in normal runs; SOFR revisions automatically supply first_changed_sofr_date.
...
A live local ThetaData/FRED/Streamlit end-to-end production run completed successfully on July 13, 2026.
The run refreshed SOFR, recalculated implied variance from July 2, 2026 through the July 13 target date,
published the canonical Hybrid v2 outputs transactionally, and finished with final health status PASS.
...
Patch 1.0.10 - SOFR refresh, lineage, and recovery repair
---------------------------------------------------------
A standalone vrp_sofr_eod_update_v1.py step now refreshes the exact canonical FRED SOFR cache before
implied variance. The updater writes a validated manifest containing rows added, rows revised, the earliest
changed observation, backup information, and audit paths. Runtime configuration now identifies the canonical
SOFR file explicitly rather than selecting the latest wildcard match, so timestamped backups cannot be mistaken
for production data.

The health gate now evaluates SOFR against the latest observation expected to have been published by the prior
completed XNYS session and validates duplicate dates, invalid rates, and missing expected sessions. When FRED
adds or revises history, the pipeline uses first_changed_sofr_date as a recalculation dependency and replaces the
affected implied-variance date x tenor rows before rebuilding downstream forecasts and signals. The new
--recalc-start option is reserved for controlled incident recovery when the SOFR cache was refreshed before the
integrated lineage logic was installed. The July 13, 2026 production repair used --recalc-start 20260702 and
completed with transactional publication and final health status PASS.
