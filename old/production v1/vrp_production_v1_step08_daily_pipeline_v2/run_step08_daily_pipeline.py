...
def load_full_inventory_report_if_available(report: Dict[str, Any]) -> Dict[str, Any]:
    """
    Step 01 prints a compact JSON summary to stdout, while the complete
    inventory is written to the json_report path.  The wrapper needs the
    complete inventory to compare downstream file dates.  This helper is
    intentionally tolerant: if the full file is not available, keep using
    the compact report.
    """
    if "downstream_file_inventory" in report and "repaired_term_structure_check" in report:
        return report
    json_report = (report.get("reports") or {}).get("json_report")
    if not json_report:
        return report
    p = Path(json_report)
    if not p.exists():
    ...
    try:
        ...
        # Preserve useful compact stdout fields if absent in the full report.
...
def latest_date_from_inventory(report: Dict[str, Any], name: str) -> Optional[str]:
    report = load_full_inventory_report_if_available(report)
...
def term_latest_date(report: Dict[str, Any]) -> Optional[str]:
    # Compact Step 01 stdout exposes this directly. Full Step 01 JSON stores
    # it under repaired_term_structure_check.latest_date.
    return (
        report.get("latest_term_structure_date")
        or report.get("repaired_term_structure_check", {}).get("latest_date")
    )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="VRP Production v1 Step 08 daily pipeline wrapper v2")
    ...
    try:
        ...
        initial = load_full_inventory_report_if_available(run_capture_json(step01_cmd, scripts["step01"].parent, log_lines))
        ...
        payload["initial_latest_term_structure_date"] = term_latest_date(initial)
        ...
        mid = load_full_inventory_report_if_available(run_capture_json(step01_cmd, scripts["step01"].parent, log_lines))
        ...
        # Final inventory
        final = load_full_inventory_report_if_available(run_capture_json(step01_cmd, scripts["step01"].parent, log_lines))
