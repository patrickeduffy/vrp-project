...
def available_curve_dates(df: pd.DataFrame) -> List[date]:
    return sorted(pd.to_datetime(df["date"]).dt.date.dropna().unique().tolist())


def reference_dates_for_selected(dates: List[date], selected_date: date) -> Dict[str, Optional[date]]:
    eligible = [d for d in dates if d <= selected_date]
    if not eligible:
        return {"Current": None, "Prior day": None, "5 trading days ago": None}
    idx = len(eligible) - 1
    return {
        "Current": eligible[idx],
        "Prior day": eligible[idx - 1] if idx - 1 >= 0 else None,
        "5 trading days ago": eligible[idx - 5] if idx - 5 >= 0 else None,
    }


def curve_slice(df: pd.DataFrame, d: date, cols: List[str]) -> pd.DataFrame:
    need = ["date", "tenor"] + cols
    out = df[pd.to_datetime(df["date"]).dt.date == d][need].copy()
...
def apply_chart_layout(fig: go.Figure, title: str, y_title: str) -> go.Figure:
    fig.update_layout(
        title=title,
        xaxis_title="Tenor / target DTE",
        yaxis_title=y_title,
        hovermode="x unified",
    ...
    fig.update_xaxes(tickmode="array", tickvals=TARGET_TENORS, ticktext=[f"{t}d" for t in TARGET_TENORS])
    return fig


def make_vix_style_comparison_chart(df: pd.DataFrame, selected_date: date) -> go.Figure:
    refs = reference_dates_for_selected(available_curve_dates(df), selected_date)
    fig = go.Figure()
    styles = {
        "Current": dict(width=4, dash="solid"),
        "Prior day": dict(width=2, dash="dash"),
        "5 trading days ago": dict(width=2, dash="dot"),
    }
    for label, d in refs.items():
        if d is None:
            continue
        curve = curve_slice(df, d, ["vix_style_vol"])
        if curve.empty:
            continue
        fig.add_trace(go.Scatter(
            x=curve["tenor"],
            y=curve["vix_style_vol"],
            mode="lines+markers",
            name=f"{label}: {d}",
            line=styles.get(label, {}),
        ))
    return apply_chart_layout(fig, "VIX-style vol term structure: current vs prior day vs 5 trading days ago", "Annualized vol (%)")


def values_are_effectively_same(a: pd.Series, b: pd.Series, tolerance: float = 1e-8) -> bool:
    aa = pd.to_numeric(a, errors="coerce")
    bb = pd.to_numeric(b, errors="coerce")
    mask = aa.notna() & bb.notna()
    if not mask.any():
        return False
    return bool((aa[mask] - bb[mask]).abs().max() <= tolerance)


def make_forecast_baseline_chart(df: pd.DataFrame, selected_date: date, smooth_display: bool = False) -> tuple[go.Figure, bool]:
    curve = curve_slice(df, selected_date, ["vix_style_vol", "forecast_vol", "trailing_realized_vol", "rv21d"])
    fig = go.Figure()
    line_shape = "spline" if smooth_display else "linear"
    same_forecast_and_realized = False
    if {"forecast_vol", "trailing_realized_vol"}.issubset(curve.columns):
        same_forecast_and_realized = values_are_effectively_same(curve["forecast_vol"], curve["trailing_realized_vol"], tolerance=1e-6)

    # Current production v1 uses trailing realized vol as the simple baseline forecast. When those two columns
    # are identical, plot one line with an explicit label instead of showing two overlapping lines.
    fig.add_trace(go.Scatter(
        x=curve["tenor"],
        y=curve["vix_style_vol"],
        mode="lines+markers",
        name="VIX-style implied vol",
        line=dict(width=4, dash="solid", shape=line_shape),
    ))
    if same_forecast_and_realized:
        fig.add_trace(go.Scatter(
            x=curve["tenor"],
            y=curve["forecast_vol"],
            mode="lines+markers",
            name="Forecast baseline = trailing realized vol",
            line=dict(width=3, dash="dash", shape=line_shape),
        ))
    else:
        fig.add_trace(go.Scatter(
            x=curve["tenor"],
        ...
        fig.add_trace(go.Scatter(
            x=curve["tenor"],
            y=curve["trailing_realized_vol"],
            mode="lines+markers",
            name="Trailing realized vol",
            line=dict(width=2, dash="dot", shape=line_shape),
        ))

    if "rv21d" in curve.columns and pd.to_numeric(curve["rv21d"], errors="coerce").notna().any():
        rv21d_val = float(pd.to_numeric(curve["rv21d"], errors="coerce").dropna().iloc[0])
        fig.add_trace(go.Scatter(
            x=curve["tenor"],
            y=[rv21d_val] * len(curve),
            mode="lines",
            name="RV21D regime reference",
            line=dict(width=2, dash="dot"),
        ))

    title = f"Implied vol vs forecast baseline: {selected_date}"
    return apply_chart_layout(fig, title, "Annualized vol (%)"), same_forecast_and_realized


def make_vol_spread_chart(df: pd.DataFrame, selected_date: date) -> go.Figure:
    curve = curve_slice(df, selected_date, ["vix_style_vol", "forecast_vol"])
    fig = go.Figure()
    if curve.empty:
        return apply_chart_layout(fig, f"Implied minus forecast vol: {selected_date}", "Vol points")
    curve["vol_spread_pts"] = pd.to_numeric(curve["vix_style_vol"], errors="coerce") - pd.to_numeric(curve["forecast_vol"], errors="coerce")
    colors = np.where(curve["vol_spread_pts"] >= 0, "#137333", "#c5221f")
    fig.add_trace(go.Bar(
        x=curve["tenor"],
        y=curve["vol_spread_pts"],
        name="Implied - forecast",
        marker_color=colors,
        hovertemplate="%{x}d: %{y:.2f} vol pts<extra></extra>",
    ))
    fig.add_hline(y=0, line_dash="dash", line_color="gray")
    return apply_chart_layout(fig, f"Vol spread across tenors: {selected_date}", "Implied - forecast vol points")


def make_vrp_curve_chart(df: pd.DataFrame, selected_date: date, scale: str = "Log ratio") -> go.Figure:
    curve = curve_slice(df, selected_date, ["vrp_log"])
    fig = go.Figure()
    if curve.empty:
        return apply_chart_layout(fig, f"VRP curve: {selected_date}", "VRP")
    if scale == "Percent premium":
        y = (np.exp(curve["vrp_log"]) - 1.0) * 100.0
        y_title = "VRP premium vs forecast (%)"
        hover = "%{y:.2f}%"
    else:
        y = curve["vrp_log"]
        y_title = "VRP log ratio"
        hover = "%{y:.4f}"
    colors = np.where(pd.to_numeric(y, errors="coerce") >= 0, "#137333", "#c5221f")
    fig.add_trace(go.Bar(
        x=curve["tenor"],
        y=y,
        name="VRP",
        marker_color=colors,
        hovertemplate="%{x}d: " + hover + "<extra></extra>",
    ))
    fig.add_hline(y=0, line_dash="dash", line_color="gray")
    return apply_chart_layout(fig, f"VRP across tenors: {selected_date}", y_title)


def make_zscore_curve_chart(df: pd.DataFrame, selected_date: date) -> go.Figure:
    curve = curve_slice(df, selected_date, ["vrp_z_3m", "vrp_z_1y"])
    fig = go.Figure()
    for label, col, dash in [("3m z-score", "vrp_z_3m", "solid"), ("1y z-score", "vrp_z_1y", "dash")]:
        fig.add_trace(go.Scatter(
            x=curve["tenor"],
            y=curve[col],
            mode="lines+markers",
    ...
    fig.add_hline(y=0, line_dash="dash", line_color="gray")
    return apply_chart_layout(fig, f"VRP z-scores across tenors: {selected_date}", "z-score")


def render_charts(project_root: Path) -> None:
    st.subheader("Term-structure charts")
    st.caption("Core visual checks: VIX-style levels, forecast/realized comparison, VRP richness, and z-scores across the 9 fixed tenors.")

    try:
        long_df, daily_df, ranking_source, ranking_exact = build_signal_history(project_root)
    except Exception as e:
        st.error(f"Could not build chart dataset: {e}")
        return

    dates = available_curve_dates(long_df)
    if not dates:
        st.warning("No chart dates found in the feature panel.")
        return

    date_options = list(reversed(dates))
    ...
    day = long_df[pd.to_datetime(long_df["date"]).dt.date == selected_date].sort_values("tenor")
    daily = daily_df[pd.to_datetime(daily_df["date"]).dt.date == selected_date]
    selected_trade = bool(daily["selected_trade_flag"].iloc[0]) if not daily.empty else False
    decision = "TRADE" if selected_trade else "NO TRADE"
    selected_tenor = daily["selected_tenor"].iloc[0] if not daily.empty else None
    selected_layer = daily["selected_layer"].iloc[0] if not daily.empty else None

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("Date", str(selected_date))
    m2.metric("Decision", decision)
    m3.metric("Selected", f"{selected_layer or '-'} / {selected_tenor or '-'}")
    m4.metric("SPX", f"{float(day['spx_close'].iloc[0]):,.2f}" if "spx_close" in day and len(day) else "-")
    m5.metric("RSI14", f"{float(day['rsi14'].iloc[0]):.1f}" if "rsi14" in day and len(day) else "-")

    tab1, tab2, tab3, tab4, tab5 = st.tabs(["VIX-style levels", "Forecast baseline", "Vol spread", "VRP", "Z-scores"])
    with tab1:
        st.plotly_chart(make_vix_style_comparison_chart(long_df, selected_date), use_container_width=True)
        st.markdown("Shows whether the implied-vol curve lifted, fell, steepened, or flattened versus the prior day and 5 trading days ago.")
    with tab2:
        smooth_display = st.checkbox("Smooth display only", value=False, help="Visual-only spline interpolation. It does not change any calculation or signal.")
        fig, same_forecast_and_realized = make_forecast_baseline_chart(long_df, selected_date, smooth_display=smooth_display)
        st.plotly_chart(fig, use_container_width=True)
        if same_forecast_and_realized:
            st.info("Current production v1 uses trailing realized vol as the simple forecast baseline, so forecast_vol and trailing_realized_vol are expected to overlap. This is not a data error; it means we have not yet swapped in an advanced forecast model.")
        st.markdown("Use this chart to compare implied vol against the current forecast baseline. RV21D is shown as a market-regime reference, not as the tenor-specific forecast.")
    with tab3:
        st.plotly_chart(make_vol_spread_chart(long_df, selected_date), use_container_width=True)
        st.markdown("This is often easier to read than plotting three vol curves: positive bars mean implied vol is above the forecast baseline by that many vol points.")
    with tab4:
        vrp_scale = st.radio("VRP scale", ["Log ratio", "Percent premium"], horizontal=True)
        st.plotly_chart(make_vrp_curve_chart(long_df, selected_date, scale=vrp_scale), use_container_width=True)
        st.markdown("This is the relative-value curve. Positive values mean implied variance is above forecast variance.")
    with tab5:
        st.plotly_chart(make_zscore_curve_chart(long_df, selected_date), use_container_width=True)
        st.markdown("The z-score curve explains why a tenor can have positive VRP but still fail the locked Core/Secondary relative-richness thresholds.")

    st.divider()
    st.subheader("Selected date signal evidence table")
    show_cols = [
        "date", "tenor", "tenor_group", "vix_style_vol", "forecast_vol", "trailing_realized_vol",
        "vrp_log", "vrp_z_3m", "vrp_z_1y", "rv21d", "rsi14", "core_pass", "secondary_pass",
        "selected", "selected_layer", "risk_fraction", "core_failed_checks", "secondary_failed_checks",
    ]
    show_cols = [c for c in show_cols if c in day.columns]
    st.dataframe(day[show_cols], use_container_width=True, hide_index=True, height=380)
...
def main() -> None:
    ...
    st.caption("Local dashboard for data health, term-structure charts, one-year audit tables, daily EOD pipeline runs, and locked 2621 signal review.")
    ...
    tab_health, tab_signal, tab_charts, tab_data, tab_run, tab_reports = st.tabs(["Health", "Signal", "Charts", "Data Explorer", "Run", "Reports"])
