...
def make_forecast_realized_chart(df: pd.DataFrame, selected_date: date) -> go.Figure:
    curve = curve_slice(df, selected_date, ["vix_style_vol", "forecast_vol", "trailing_realized_vol"])
    fig = go.Figure()
    series = [
        ("VIX-style vol", "vix_style_vol", "solid"),
        ("Forecast vol", "forecast_vol", "dash"),
        ("Trailing realized vol", "trailing_realized_vol", "dot"),
    ]
    for label, col, dash in series:
    ...
    return apply_chart_layout(fig, f"Implied vs forecast vs trailing realized vol: {selected_date}", "Annualized vol (%)")
...
def render_charts(project_root: Path) -> None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["VIX-style levels", "Forecast vs realized", "VRP", "Z-scores"])
    with tab1:
    ...
    with tab2:
        st.plotly_chart(make_forecast_realized_chart(long_df, selected_date), use_container_width=True)
        st.markdown("Compares the current implied curve against the production forecast-vol denominator and trailing realized vol context.")
    with tab3:
